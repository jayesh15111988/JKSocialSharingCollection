...
- (void)setupFacebookLogin {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:FBSDKAccessTokenDidChangeNotification object:nil queue:nil usingBlock:^(NSNotification *notification) {
        //self.facebookShareButton.hidden = ![FBSDKAccessToken currentAccessToken];
        if ([FBSDKAccessToken currentAccessToken]) {
            [self fetchFacebookAccountInfoWithCurrentToken];
        }
    }];
...
- (void)setupFacebookSharing {
    ...
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[_facebookLoginButton]-10-[_facebookShareButton(44)]" options:kNilOptions metrics:nil views:NSDictionaryOfVariableBindings(_facebookLoginButton, _facebookShareButton)]];
}
...
- (IBAction)googlePlusShareButtonPressed:(id)sender {
    if ([[GPPSignIn sharedInstance] hasAuthInKeychain]) {
        ...
        // You cannot both attach image and set URL to share. Only one of those action is allowed by Google standard.
        UIImage* rfImage = [UIImage imageNamed:@"rf.jpg"];
        [shareBuilder attachImage:rfImage];
        
        // Make sure setURLToShare and setCallToActionButtonWithLabel are called together.
...
- (void)setupTwitter {
    ...
    if ([[NSUserDefaults standardUserDefaults] valueForKey:OAUTH_TOKEN_STORE_KEY]) {
...
- (void)setupTwitterLogin {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"update_token" object:nil queue:nil usingBlock:^(NSNotification *notification) {
        ...
        NSString* oauthTokenVerifier = twitterTokenInfo[@"oauth_verifier"];
        [self setOAuthToken:oauthToken oauthVerifier:oauthTokenVerifier];
        self.twitterLoginButton.hidden = YES;
    }];
...
- (void)setupTwitterSharing {
    ...
    if (self.sharingMediaContentOnTwitter) {
        ...
        [_twitter postStatusUpdate:self.twitterShareText inReplyToStatusID:nil mediaURL:fileURL placeID:nil latitude:nil longitude:nil uploadProgressBlock:^(NSInteger bytesWritten, NSInteger totalBytesWritten, NSInteger totalBytesExpectedToWrite) {
        ...
        [self.twitter postStatusUpdate:self.twitterShareText inReplyToStatusID:nil latitude:nil longitude:nil placeID:nil displayCoordinates:nil trimUser:nil successBlock:^(NSDictionary *status) {
...
- (void)logErrorWithErrorObject:(NSError*)error {
    NSLog(@"Failed to tweet staus with Error code %ld and Error description %@ and Error domain is %@", [error code], [error localizedDescription], [error domain]);
}
